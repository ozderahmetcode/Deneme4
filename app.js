...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.toggleRoomMicMode = function () {
        ...
        if (roomMicMode === 'open') {
            ...
            if (roomClient) roomClient.setMute(true); // Start muted
        } else {
            roomMicMode = 'open';
            btn.innerText = "Mod: Open Mic";
            pttBtn.classList.add('hidden');
            if (roomClient) roomClient.setMute(false); // Enable mic
        }
    }
    ...
    if (pttBtn) {
        ['mousedown', 'touchstart'].forEach(evt => {
            pttBtn.addEventListener(evt, () => {
                if (roomMicMode === 'ptt' && roomClient) {
                    roomClient.setMute(false);
                    pttBtn.style.background = 'var(--green)';
                }
            });
        });
